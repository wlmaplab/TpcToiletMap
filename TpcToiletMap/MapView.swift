...
struct MapView: UIViewRepresentable {
    typealias UIViewType = ToiletMapView
...
class ToiletMapView: MKMapView, MKMapViewDelegate {
...
//            print("緯度:\(location.coordinate.latitude), 經度: \(location.coordinate.longitude)")
    ...
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        if annotation is MapPin {
            ...
            setCalloutViewWith(annotationView: annoView, text: "\(anno.address ?? "")\(anno.info != nil ? "\n\(anno.info!)" : "")")
    ...
    @objc func openAppleMaps() {
        ...
        mapItem1.name = "現在位置"
        mapItem2.name = self.selectedAnnotation?.address ?? "目標公廁"
